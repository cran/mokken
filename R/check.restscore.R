...
function(X, minvi = .03, minsize = default.minsize){
  ...
  for (i in 1:(J-1)){
     ...
     for (j in (i+1):J){
        ...
        # dimnames(z.scores)[[2]] <- list(outer(paste("P(X",i,">=",1:(m-1),")",sep=""),paste("P(X",j,">=",1:(m-1),")",sep=""),paste))
        # if(nrow(z.scores) > 1) dimnames(z.scores)[[1]] <- list(paste("Rest-score", apply(summary.matrix[,2:3],1,paste, collapse="-")))
