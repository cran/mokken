check.errors <- function(X, returnGplus = TRUE, returnOplus = FALSE){
  ...
  if(returnOplus) {
    #### begin bugfix ####
    num.item.points <- max(sapply(1:ncol(X), function(i) length(unique(X[,i])))) 
    counting.mat <- apply(X + 1, 2, function(x) tabulate(x, nbins = num.item.points))
    O <- apply(apply(counting.mat, 2, rank), 2, rev) - 1
    #### end bugfix ####
    OScores <- X
...
"check.errors.old" <- function(X, returnGplus = TRUE, returnOplus = FALSE){
