...
# - type.z argument replaces type.se: "Z" = (Mokken's) Z-score zoals origineel geprogrammeerd, 
...
function(X, lowerbound, alpha, StartSet, verbose, type.z, test.Hi, level.two.var){
  ...
  # Internal functions
   ...
   if(any(is.na(StartSet)) | any(is.nan(StartSet)) | (is.logical(StartSet) & any(StartSet == TRUE))){
      warning("Start set of items is not properly defined. User-defined StartSet ignored.")
      StartSet <- FALSE
   }   
   if(!(is.logical(StartSet) | is.numeric(StartSet)| is.integer(StartSet))){
      warning("StartSet should be logical or numeric. User-defined StartSet ignored.")
      StartSet <- FALSE
   }   
   StartSet <- unique(StartSet)
   if (!is.logical(StartSet) & all(StartSet %in% 1 : ncol(X)) == FALSE){
      warning("Start set of items is not properly defined. User-defined StartSet ignored")
      StartSet <- FALSE
   }   
   startset.provided <- FALSE
   if (!is.logical(StartSet)){
   ...
   for (lb in lowerbound){
      ...
      repeat{
        ...
        if (is.logical(StartSet) | scale > 1){
        ...
        if (!is.logical(StartSet)) StartSet <- sort(StartSet)
